...
#define RSHA256_SCR(i)                                                              \
    do {                                                                            \
        w[i] = RSHA256_F4(w[i - 2]) + w[i - 7] + RSHA256_F3(w[i - 15]) + w[i - 16]; \
    } while (0)

#define RSHA256_EXP(a, b, c, d, e, f, g, h, j)                                          \
    do {                                                                                \
        x1 = wv[h] + RSHA256_F2(wv[e]) + CH(wv[e], wv[f], wv[g]) + rsha256_k[j] + w[j]; \
...
void rsha256(unsigned char const *message, unsigned int len, unsigned char *digest)
{
    struct T ctx;
    rsha256_init(&ctx);
